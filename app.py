...
def scrape_product_details(url):
    ...
    try:
        ...
        # 10. SIZES - WooCommerce variations se
        sizes = []
        ...
        # Agar select nahi mila toh variation buttons try karo
        if not sizes:
        ...
        if sizes:
            ...
            # Agar size nahi mila toh ek blank row
            supplemental_rows.append({
                "id": product_id,
                "title": title,
                "color": color,
    ...
    if not urls:
        ...
        with st.spinner("Google Sheet se connect ho raha hai..."):
            try:
                ...
                primary_sheet.clear()
                primary_sheet.append_row(primary_fieldnames)
                supplemental_sheet.clear()
                supplemental_sheet.append_row(supplemental_fieldnames)
                sheet_connected = True
                st.success("✅ Google Sheet connected! (Primary Feed + Supplemental Feed tabs ready)")
            except Exception as e:
        ...
        for i, url in enumerate(urls, 1):
            ...
            if primary_data:
                ...
                if sheet_connected:
                    # Primary Feed
                    row = [primary_data[f] for f in primary_fieldnames]
                    primary_sheet.append_row(row)
                    # Supplemental Feed
                    for s_row in supplemental_rows:
                        supplemental_sheet.append_row([s_row[f] for f in supplemental_fieldnames])
